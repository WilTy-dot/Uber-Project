...
```{r Packages loading and data import/merging, echo=TRUE, warning=FALSE}

rm(list = ls(all = TRUE)) # clears memory

if(!require(pacman))install.packages("pacman")
p_load(geosphere,ggplot2,dplyr,readr,geosphere,distm, data.table,esquisse,corrplot,tibble,lubridate,stargazer,vis_dat,readr,visdat)
...
```
```{r Filtering the merged training set (raw data)to account for pick up with 1km of NYSE, echo=TRUE, message=FALSE, warning=FALSE, paged.print=FALSE}
...
```{r Creating a vector containing all relevant timepoints with 15 minutes gap, echo=TRUE, message=FALSE, warning=FALSE, paged.print=FALSE}
...
```{r Creating 15min-interval objects, echo=TRUE, message=FALSE, warning=FALSE, paged.print=FALSE}
time_intervals<-int_diff(time_points)
```



```{r Matching the raw data time points to interval objects, echo=TRUE, message=FALSE, warning=FALSE, paged.print=FALSE}
...
```{r filtering the raw data to match the period [ 17:00 to 00:00] and computing the number of rides per interval object}
...
```{r Adding new features namely ,wday, federal holiday, weekend, timespan, Mon,Tue,Wed,Thu,Fri, Sat, Sun and hour_numeric. federal holiday, weekend, Mon,Tue,Wed,Thu,Fri, Sat, Sun are boolean features (0,1) and hour_numeric represents the time in minutes from 17:00.}
...
```{r Distribution of pick up demand per wday}
...
```{r Distribution of ride demand per timespan}
ggplot(uber_train_comb_fin) +
...
```{r Distribution of ride demand per weekday}
ggplot(uber_train_comb_fin) +
 aes(x = timespan, y = number_of_rides_per_interval) +
...
```{r Model 1: The number_of_rides trend in the evening}
mod1<-lm(number_of_rides_per_interval~hour_numeric, data=uber_training)
summary(mod1)

```

```{r Model 2: The number_of_rides trend in the evening + each day impact}
mod2<-lm(number_of_rides_per_interval~hour_numeric+Mon+Tue+Wed+Thu+Fri+Sat,
         data=uber_training)
summary(mod2)
```

```{r Model 3: The number_of_rides trend in the evening + each day impact + interaction between each day and evening time}
...
```{r Model 4: The number_of_rides trend in the evening + weekend/weekday impact + interaction between weekend/weekday and evening time}
...
```{r Model 1: MAPE(Mean Absolute Percentage Error)}
...
```{r Model 2: MAPE(Mean Absolute Percentage Error)}
...
```{r Model 3: MAPE(Mean Absolute Percentage Error)}
...
```{r Model 4: MAPE(Mean Absolute Percentage Error)}
...
### Conclusion: Model 3 displays the best results in term of accuracy (MAPE=43.5%)nad adjusted R2= 55%, therefore it will used to predict the number of rides
...
```{r Importing S&P500 daily data from 2014-04-01 to 2014-07-31}
install.packages('quantmod')
library(quantmod)
#remotes::install_github("joshuaulrich/quantmod@358-getsymbols-new.session")4
...
```{r Extracting the daily returns and transforming the table}
...
```{r Merging train_transformed set & S&P500 daily returns from 2014-04-01 to 2014-07-31}
uber_filtered_SP500<-merge.data.table(uber_train_comb_fin,RetDF, by='Date', all.x = TRUE)
```

```{r Visualizing the merged data and treating NAs }
...
```{r Impact of S&P500 on uber pick up demand}
ggplot(uber_filtered_SP500) +
 aes(x = intervals, y = number_of_rides_per_interval, color= daily.returns ) +
 geom_line() +
...
```{r Model 1: The number_of_rides trend with SP500 impact}
mod1_SP500<-lm(number_of_rides_per_interval~daily.returns, data=uber_SP500_training)
summary(mod1_SP500)
```


```{r Model 2: The number_of_rides trend in the evening with SP 500 impact }
mod2_SP500<-lm(number_of_rides_per_interval~daily.returns+hour_numeric*(Mon+Tue+Wed+Thu+Fri+Sat), data=uber_SP500_training)
summary(mod2_SP500)
...
####Conclusion: SP did not help improving the initial model
...
```{r Adding hour_numeric and week days features to the test set}
...
uber_test$wDay<-lubridate::wday(uber_test$Time_Interval,label = TRUE, abbr = FALSE)


uber_test$Mon<-ifelse(uber_test$wDay=="Monday",1,0)
uber_test$Tue<-ifelse(uber_test$wDay=="Tuesday",1,0)
uber_test$Wed<-ifelse(uber_test$wDay=="Wednesday",1,0)
uber_test$Thu<-ifelse(uber_test$wDay=="Thursday",1,0)
uber_test$Fri<-ifelse(uber_test$wDay=="Friday",1,0)
uber_test$Sat<-ifelse(uber_test$wDay=="Saturday",1,0)
```

```{r Importing S&P500 daily data from 2014-09-01 to 2014-09-30}
getSymbols(c('^GSPC'),
           from='2014-09-01',
           to='2014-09-30')
```
...
```{r Adding Date, Merging the test set with SP500 daily returns and impute calculatated values to NA's}
...
prediction.test <- predict(mod3, uber_test)
